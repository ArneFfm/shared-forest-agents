"""World Forest SDK. Zero dependencies (urllib only).
...
__all__ = ["DEFAULT_BASE_URL", "WorldForestClient", "WorldForestError"]
__version__ = "0.1.0"

DEFAULT_BASE_URL = "https://world.ghardenlab.com"


class WorldForestError(Exception):
    ...
    def __init__(self, status, problem=None, message=None):
        ...
        super().__init__(message or detail or "World Forest API answered HTTP %d" % status)
        self.status = status
...
class WorldForestClient:
    """Client for the World Forest REST API (version 1).
    ...
    def __init__(self, base_url=DEFAULT_BASE_URL, timeout=30.0, transport=None, user_agent=None):
        ...
        self.user_agent = user_agent or "world-forest-sdk-python/" + __version__
    ...
    def request(self, method, path, query=None, body=None, idempotency_key=None):
        ...
        try:
            ...
            parsed = None
        if status >= 400:
            raise WorldForestError(status, parsed)
        return parsed
    ...
    def start_sponsorship(self, quantity, design_ids, display_name, email, idempotency_key, spots=None, hold_ids=None, link=None):
        ...
        if spots:
            ...
            if held.get("rejected"):
                ...
                raise WorldForestError(409, None, "Spots not free: " + reasons)
    ...
    def wait_for_order(self, order_id, interval=10.0, timeout=35 * 60, sleep=time.sleep):
        ...
        while True:
            ...
            if time.monotonic() + interval > deadline:
                raise WorldForestError(408, None, "Order %s is still %s after %s s." % (order_id, order["status"], timeout))
